...
export default function ChangeEmailForm({ email, setShowModal, toastRef, setRelodUser }) {
    ...
    const onSubmit = async () => {
        ...
        if (!resultUpdateEmail.statusResponse) {
            setErrorEmail("No se puedo cambiar por este correo, ya esta en uso por otro usuario")
            return
        }
        setRelodUser(true)
    ...
    const validateForm = () => {
        ...
        if (!validateEmail(newEmail)) {    
            setErrorEmail("Debes ingresar un mail valido")
            isValid = false
        }
        if (newEmail === email) {
            setErrorEmail("Debes ingresar un emal diferente al actual")
            isValid = false
        }
